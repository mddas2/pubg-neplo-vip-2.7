...
#define XTREME_DRIVER_FILE  "mddascorn.sys"
#define XTREME_SERVICE_NAME "mddascorn"
#define XTREME_DEVICE_NAME  "\\Device\\mddascorn"
...
std::string name = "PRO"; // application name. right above the blurred text aka the secret on the licenses tab among other tabs
std::string ownerid = "oxPiP15arZ"; // ownerid, found in account settings. click your profile picture on top right of dashboard and then account settings.
std::string secret = "701717156574e798c454438fdeba90b8cc864c886593b0123339b5c4b4a26d9a"; // app secret, the blurred text on licenses tab and other tabs
...
auto GetExpiry = [=]()
{
	...
	return
		std::to_string(Days) + " D | " + std::to_string(Hours) + " H | " +
		std::to_string(Minutes) + " M  " + std::to_string(Seconds) + " S.";

};

bool urlFileExists(const std::string& url) {
	CURL* curl = curl_easy_init();
	if (curl) {
		curl_easy_setopt(curl, CURLOPT_URL, url.c_str());
		curl_easy_setopt(curl, CURLOPT_NOBODY, 1L); // Only fetch the header, not the body
		...
		if (res == CURLE_OK) {
			...
			if (response_code == 200) {
				...
				return true; // URL file exists
			}
		}

		curl_easy_cleanup(curl);
	}
	return false; // URL file does not exist or other error occurred
}
...
bool login() {
	...
	if (FileExist(("C:\NEPLO VIP.lic")))
	{
		std::ifstream MyFile(("C:\NEPLO VIP.lic"));
		...
		login = KeyAuthApp.data.success;

		//std::cout << termcolor::bright_white << "-----------------------------------\n";
		std::cout  << "License : " << token << std::endl;
		Sleep(500);

		std::cout << "Expiry: " << (XorStr(" %s").c_str(), GetExpiry().c_str());
		Sleep(500);
	...
	again:
		...
		if (login) {
			...
			std::cout << "\n...............................................\n";
			Sleep(1000);
			std::ofstream MyFile(("C:\NEPLO VIP.lic"));
			MyFile << token;
			MyFile.close();
			return true;
		}
...
int main()
{
	...
	SetConsoleTitle((L"Aorus VIP"));
	system("Color F");
	system("MODE Con Cols=56 lines=16");

	print(XorStr("Welcome back to Aorus VIP Hax...\n"), 4);
	
	std::cout << "Version : " << version << std::endl;


	//std::string url = "http://64.227.182.105:8004/media/signature/doc.pdf"; // Replace with the URL of the file you want to checks
	/*if (urlFileExists(url)) {
		std::cout << "Checking New Updates...." << std::endl;
	}
	else {
		std::cout << "It can't Update Please contact https://discord.gg/pCCFu72tRD" << std::endl;
		std::system("pause");
		exit(1);
		std::cout << "URL file does not exist." << std::endl;
	}*/
	print(XorStr("Please visit discord for latest update https://discord.gg/pCCFu72tRD\n"), 4);
	//login();
	Sleep(1000);

	print(XorStr("[+]Checking for key... \n"), 4);
...
//	logfk();
	...
	{
		m_drv->Unload();
		MessageBox(NULL, L"Can't Find Game Window", L"mddascorn", MB_OKCANCEL);
		system("pause >nul");
		ExitProcess(0);
	}
	...
	D3DXCreateTextureFromFileInMemory(DX11::pDevice, rawData, sizeof(rawData),  &DX11::Image ); 
	...
	DX11::ImGui_DX11::pRegularFont = Io.Fonts->AddFontFromMemoryCompressedTTF(font_awesome_data, font_awesome_size, 16.0f, &icons_config, icons_ranges);
	DX11::ImGui_DX11::pRegularFont = Io.Fonts->AddFontFromFileTTF("C:\\windows\\Fonts\\arial.ttf", 16.0f, &icons_config, icons_ranges);
